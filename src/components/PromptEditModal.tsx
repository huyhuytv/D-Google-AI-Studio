...
export const PromptEditModal: React.FC<PromptEditModalProps> = ({ isOpen, prompt, onSave, onClose }) => {
    ...
    return (
        <div className={`fixed inset-0 bg-black/60  flex items-center justify-center z-50 p-4   ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={onClose}>
            <div ref={modalRef} className={`bg-slate-800 border border-slate-700 rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col   ${isOpen ? 'scale-100' : 'scale-95'}`} onClick={e => e.stopPropagation()}>
